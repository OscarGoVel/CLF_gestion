        ...
        try:
            ...
            for prod in productos_entregar:
                ...
                """, (
                    ...
                    fecha_entrega,
                    notas,
                    'Sistema'  # Aquí podrías poner el usuario actual
                ))
...
class SistemaGestion:
    ...
    # ── SECCIÓN: DASHBOARD ─────────────────────────────────────────────────
    def _crear_seccion_dashboard(self):
        sec = tk.Frame(self._content_area, bg=self.C['content_bg'])
        self._secciones['dashboard'] = sec

        # Toolbar
        tb = tk.Frame(sec, bg=self.C['toolbar_bg'], relief='flat', bd=0)
        tb.pack(fill='x')
        tk.Frame(sec, bg=self.C['toolbar_border'], height=1).pack(fill='x')
        self._toolbar_btn(tb, '🔄 Actualizar', self.actualizar_dashboard)
        ...
        for i, (estado, fg, bg) in enumerate(estados_cfg):
            ...
            def _ir_estado(e=None, _est=estado):
                self._filtro_estado_cot.set(_est)
                self._navegar('cotizaciones')
            bg_dark = _darken(bg)
