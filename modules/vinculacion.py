...
# RFC propio — se excluye de deteccion de proveedores
...
def detectar_pendientes(cursor):
    """Escanea facturas y retorna dict de pendientes por categoria."""
    pendientes = {
        "clientes":    [],
        "proveedores": [],
        "productos":   [],   # no_id no encontrado en catalogo por codigo
        "cotizaciones":[],
    }

    # Clientes: RFC receptor no registrado en catalogo de clientes
    # Se excluye CLF_RFC propio (no debe aparecer como cliente sin registrar)
    cursor.execute("""
    ...
    # Un concepto del XML se considera resuelto si:
    #   - Su no_identificacion coincide con algún productos.codigo, O
    #   - Su clave_prod_serv coincide con alguna entrada en producto_claves_sat
    #     (tabla que acumula todas las claves SAT históricas de cada producto)
    # Se agrupa por el identificador principal (no_identificacion si tiene, si no clave_prod_serv)
    cursor.execute("""
    ...
    # Facturas de VENTA sin cotización vinculada
    # (CLF es el emisor → facturas que tú emites a clientes)
    cursor.execute("""
    ...
    # Facturas de COMPRA sin registro en compras
    # (CLF es el receptor → facturas de proveedores)
    # Se marcan si aún no tienen un registro en la tabla compras (factura_xml_id)
    cursor.execute("""
...
# ── Carga completa de registros para el dialogo ────────────────────────────────

def _cargar_clientes(cursor):
    cursor.execute("""
...
def _cargar_proveedores(cursor):
    cursor.execute("""
