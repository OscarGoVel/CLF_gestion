...
"""
web_app/cfdi.py
Parser de archivos CFDI 3.3 / 4.0 — sin dependencias de UI.
"""
...
def parsear_cfdi(ruta_xml: str) -> dict:
    ...
    Retorna dict con todos los campos relevantes.
    Lanza ValueError si el archivo no es un CFDI válido o no está timbrado.
    """
    try:
...
def parsear_cfdi_bytes(contenido: bytes) -> dict:
    ...
    # Reutilizamos la misma lógica escribiendo a un archivo temporal en memoria
    import tempfile, os
    with tempfile.NamedTemporaryFile(delete=False, suffix=".xml") as tmp:
        tmp.write(contenido)
        tmp_path = tmp.name
    try:
        return parsear_cfdi(tmp_path)
    finally:
        os.unlink(tmp_path)
